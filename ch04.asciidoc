...
We noted earlier that our user relationship model is a natural graph and that our attachments might be well-served from a key/value store, so <<>> will take a look at implementing persistent storage through these mechanisms.
