...
You'll be prompted for your +$JAVA_HOME+ location and JBoss AS7 version; be sure to align the versions with the Arquillian Container Adaptor Version we'd chosen before.  Again, in this example we recommend +7.1.1.Final+.  Forge will additionally ask for the location to a JBoss AS7 installation on the filesystem, but simply hitting Enter will download the server for us into the +target+ directory of our project.
...
If you've opted for a different version of JBoss AS7, you may have to make substitutions to point +JBOSS_HOME+ correctly.  Assuming all goes as planned, you should see the JBoss AS7 startup sequence in the Forge shell, followed by:
