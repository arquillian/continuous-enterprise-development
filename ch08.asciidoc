...
|+CookieParam+|Binds the value of an HTTP cookie to a resource method parameter, resource class field, or resource class bean property.
...
|+Encoded+|Disables automatic decoding of parameter values bound using `QueryParam`, `PathParam`, pass:[<phrase role='keep-together'><literal>FormParam</literal></phrase>], or `MatrixParam`.
...
*** +GET+ -> List
*** +POST+ -> Add
** pass:[<phrase role='keep-together'><literal>/conference/[c_id/session/[s_id] application/vnd.ced+xml;</literal></phrase>] pass:[<phrase role='keep-together'><literal>type=session</literal></phrase>]
*** +GET+ -> Single
*** +PUT+ -> Update
*** +DELETE+ -> Remove
...
Let's have a look at a concrete implementation of the +RepositoryResource+, one that handles interaction with +User+ domain objects.  We've aptly named this the +org.cedj.+ pass:[<phrase role='keep-together'><literal>geekseek.web.rest.user.UserResource</literal></phrase>]:
...
((("Warp (Arquillian)","frameworks")))Warp currently focuses on frameworks based on the Servlets API, but it provides special hooks and additional support for:
...
((("test harness setup in Warp")))((("Warp (Arquillian)","test harness setup")))We'll start by enabling the Arquillian Warp in the POM's +dependencyManagement+ pass:[<phrase role='keep-together'>section:</phrase>]
