...
That is precisely the aim of the ShrinkWrap Resolvers project; it is a Java API to obtain artifacts from a repository system.  Currently implemented are grammars and support for Maven-based respository structures (this is separate from the use of Maven as a project management system or build tool; it's possible to use a Maven repository layout with other build systems).
...
Often, we have a Maven _POM_ (Project Object Model) file which contains dependency information about our current project.  In many cases it might be more desireable to pull information from there rather than hardcode and potentially duplicate dependency information in our code.  Therefore ShrinkWrap Resolvers supports the "loading" of POM metadata from a +File+ or +ClassLoader+ resource:
