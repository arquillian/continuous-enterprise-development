...
First we'll log in to our CloudBees account and select to enter the Jenkins Dashboard from within CloudBees Central, as shown in <<Figure10-3>>.
...
Once the gem is installed, you can add the domain record to OpenShift using the command ++rhc alias add _appName alias_ -l _username_++. For instance:
