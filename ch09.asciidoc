...
Now we can move to the business of authenticating a user via the Twitter OAuth service via Agorava.  We can extend PicketLink's +BaseAuthenticator+ to provide the necessary logic in our +org.cedj.geekseek.service.security.picketlink.OAuth+ pass:[<phrase role='keep-together'><literal>Authenticator</literal></phrase>]:
